...
public class FurnitureCreate : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler, IPointerDownHandler
{
    ...
    void Start()
    {
        ...
        foreach (Furniture f in ButtonManager.furniture)
        {
            if (childname == f.modelID)
            {
                thiswidth = (float)f.width;
                thislength = (float)f.length;
            }
        }
    ...
    public void OnBeginDrag(PointerEventData eventData)
    {
        ...
        else if (state == 1)
        {
            ...
            thislength *= transform.localScale.y;
            thiswidth *= transform.localScale.x;
        }
        else if (state == 2)
        {
